$(document).ready(function() {
	...
	$("#btnContactUs").click(function(){
			$("#contact_popup").show(1000);
		
	});
	$("#btnClose").click(function(){
		$("#contact_popup").hide(1000);
...
function threeBoxSlidingEffect(){
	if($.browser.msie && ($.browser.version == 7.0 || $.browser.version == 8.0)){
		$(".home-three-box-sliding-image").parent().mouseenter(function(){
			...
			animate({backgroundPositionY: '-90px'}); 
		}).mouseleave(function(){
...
function _date(){
	var days = new Array();
	days = ["Sun day","Mon day","Tues day","Wednes day","Thurs day","Fri day","Satur day"];
	var months = new Array();
	months = ["January","February","March","April","May","June","July","August","September","October","November","December"];
	var date = new Date();
	var _dt = parseInt(date.getDate());
	var _substr = "";
	if(_dt==1 || _dt==31 || _dt==21)
	_substr = "st";
	if(_dt==2 || _dt==22 )
	_substr = "nd";
	if(_dt==3 || _dt==23)
	...
	if(_dt>23 || _dt==20)
	_substr="th";
	var fullDate = String(_dt)+_substr+"&nbsp;"+String(months[date.getMonth()])+"&nbsp;"+String(date.getFullYear());
	$("#dateHolder").html(fullDate);
}
